...
def summarize_command(args: argparse.Namespace) -> int:
    ...
    emails, account_results = fetch_accounts_for_date(
        ...
        per_account_limit=args.max_emails or agent_config.max_emails,
        workers=agent_config.fetch_workers,
    )
    failed = [item for item in account_results if not item["ok"]]
    if failed:
        print("Some mailboxes failed:")
        for item in failed:
            print(f"- {item['address']}: {item['error']}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    summarize = subparsers.add_parser("summarize", help="Summarize all configured mailboxes for a date.")
    summarize.add_argument("--date", default="today", help="today or YYYY-MM-DD. Default: today.")
    summarize.add_argument("--max-emails", type=int, default=None, help="Maximum emails to read per mailbox.")
